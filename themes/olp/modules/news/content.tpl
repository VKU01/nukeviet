...
<div class="margin-top margin-bottom">
	...
	<a class="btn btn-primary"
		href="{BASE_URL}&amp;contentid=0&checkss={ADD_CONTENT_CHECK_SESSION}">{LANG.add_content}</a>
</div>
...
<form action="{FORM_ACTION}" method="post" onsubmit="formSubmit(event, this)">
	...
	<div class="table-responsive">
		<table id="edit" class="table table-striped table-bordered table-hover">
			<tfoot>
				<tr>
					<td class="text-center" colspan="2"><input class="btn btn-primary frm-item" name="submit"
							type="submit" value="{LANG.author_info_save}" /></td>
				</tr>
			</tfoot>
			<tbody>
				<tr>
					<td class="text-right text-nowrap"><strong>{LANG.author_pseudonym}: </strong> <sup
							class="required">(*)</sup></td>
					<td>
						<div class="form-group"><input class="form-control frm-item" name="pseudonym" id="pseudonym"
								type="text" value="{DATA.pseudonym}" maxlength="100"
								data-mess="{LANG.author_pseudonym_empty}" /></div>
					</td>
				</tr>
				<tr>
					<td class="text-right text-nowrap"><strong>{LANG.author_description}: </strong></td>
					<td><textarea class="frm-item form-control" id="description" name="description"
							rows="10">{DATA.description_br2nl}</textarea></td>
				</tr>
			</tbody>
		</table>
	</div>
...
<div>
	<span class="pull-right"><i class="fa fa-arrow-right"></i>&nbsp;<a
			href="{AUTHOR_PAGE_URL}">{LANG.author_page}</a></span>
...
<div class="margin-top margin-bottom">
	<a class="btn btn-primary" href="{BASE_URL}">{LANG.your_content}</a>&nbsp;
	<!-- BEGIN: add_content --><a class="btn btn-primary"
		href="{BASE_URL}&amp;contentid=0&checkss={ADD_CONTENT_CHECK_SESSION}">{LANG.add_content}</a>&nbsp;
	<!-- END: add_content -->
	<a class="btn btn-primary" href="{BASE_URL}&amp;author_info=1">{LANG.author_info}</a>
</div>
...
<form action="{CONTENT_URL}" enctype="multipart/form-data" name="fsea" method="post" id="fsea" class="form-horizontal" <!-- BEGIN: recaptcha3 -->
	data-recaptcha3="1"
	<!-- END: recaptcha3 -->>
	...
	<div class="form-group">
		...
		<div class="col-sm-16">
			<input type="text" class="form-control pull-left" name="alias" id="idalias" value="{DATA.alias}"
				maxlength="255" style="width: 94%;" />
			<em class="fa fa-refresh pull-right" style="cursor: pointer; vertical-align: middle; margin: 9px 0 0 4px"
				onclick="get_alias('{OP}');" alt="Click">&nbsp;</em>
		</div>
	</div>
	<!-- END: alias -->

	<div class="form-group">
		<label class="col-sm-8 control-label text-normal">{LANG.content_cat} <span
				class="txtrequired">(*)</span>:</label>
		<div class="col-sm-16">
			<div
				style="height: 130px; width: 100%; overflow: auto; text-align: left; border: solid 1px #ddd; padding: 11px;">
				<table>
					<tbody>
						<!-- BEGIN: catid -->
						<tr>
							<td><input class="news_checkbox" name="catids[]" value="{DATACATID.value}" type="checkbox"
									{DATACATID.checked}>{DATACATID.title}</td>
						</tr>
						<!-- END: catid -->
					</tbody>
				</table>
	...
	<div class="form-group">
		...
		<div class="col-sm-24"><textarea class="form-control" rows="6" cols="60"
				name="hometext">{DATA.hometext}</textarea></div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-16">
			<input type="text" maxlength="6" value="" id="fcode_iavim" name="fcode" class="form-control pull-left"
				style="width: 150px;" />
			<img height="32" src="{NV_BASE_SITEURL}index.php?scaptcha=captcha&t={NV_CURRENTTIME}" alt="{LANG.captcha}"
				class="captchaImg" />
			<img alt="{CAPTCHA_REFRESH}" src="{CAPTCHA_REFR_SRC}" width="16" height="16" class="refresh"
				onclick="change_captcha('#fcode_iavim');" />
		</div>
	</div>
	<!-- END: captcha -->
	...
	<div class="form-group">
		<div class="col-24">
			<div class="nv-recaptcha-default">
				<div id="{RECAPTCHA_ELEMENT}" data-toggle="recaptcha" data-pnum="3" data-btnselector="[type=submit]">
				</div>
			</div>
		</div>
	</div>
